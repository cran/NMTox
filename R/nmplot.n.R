#' Create plots of the dose and the response for every unique value of a certain
#' variable
#'
#' This function generates scatter plots of the dose and the response for every
#' unique value of a specified variable
#'
#' @usage nmplot.n(data.nm, data.control, id, nano, response, dose, end,
...
#' @return This function produces scatter plots of the dose and the response for
...
#' # provider):
#' #
#' \donttest{nmplot.n(data.nm=invitrodata, data.control=controldata, id="experimentID",
...
#' @export
nmplot.n<-function(data.nm, data.control, id, nano, response,
                       dose, end, end.cat,  unit, unit.cat,
                       cat, type=c("dose","log"),
                       control.opt=c("same","all"),
                       nrow=1,ncol=1) {
  ...
  } else
  {
    ...
    for (nano.cat in unique(data.nm[[nano]])) {
      ...
      for (u.cat in unique(df.nm[[cat]])) {
        if (is.na(u.cat)==T) {
          df.na<-df.nm[is.na(df.nm[[cat]])==TRUE,]
          plot.cat[[u.cat]]<-ggplot(df.na,
                                    aes(x=cons,y=!!resp,colour=!!cats)) +
            geom_point(shape=16,size=3)+ylab(paste(response)) +
            xlab(paste(dose.lab,"(",unit.cat,")")) +
            scale_shape_manual(values = c(1,2))+labs(colour = paste(cat))+
            scale_colour_brewer(palette = "Set1",
                                aesthetics="colour",na.value="grey45")+
            ggtitle(paste(nano.cat,u.cat,sep="\n")) + theme_bw()
        } else {
